...
MULTIVARIATE_FUNCTION=run_geospm_A_AxB_B_interaction
...
if [ "$EFFECT_SIZE_ARG" != "" ]; then

    IFS=$' \t'
    EFFECT_SIZE_TMP=($EFFECT_SIZE_ARG)
    unset IFS
    
    EFFECT_SIZE=()
    
    for i in "${!EFFECT_SIZE_TMP[@]}"; do
        EFFECT_SIZE_NUMBER="${EFFECT_SIZE_TMP[i]}"
        
        EFFECT_SIZE+=("$EFFECT_SIZE_NUMBER")
    done
    
    IFS=$' '
    unique_array_from "${EFFECT_SIZE[*]}" "Effect size numbers" EFFECT_SIZE false
    unset IFS
    
    echo "${indent}[*effect-size] "
    print_array EFFECT_SIZE
else
    echo "${indent}[effect-size] "
    print_array EFFECT_SIZE
fi
...
if [ "$REGIONALISATION_ARG" != "" ]; then

    IFS=$' \t'
    REGIONALISATION_TMP=($REGIONALISATION_ARG)
    unset IFS
    
    REGIONALISATION=()
    
    for i in "${!REGIONALISATION_TMP[@]}"; do
        REGIONALISATION_MATRIX="${REGIONALISATION_TMP[i]}"
        
        REGIONALISATION+=("$REGIONALISATION_MATRIX")
    done
    
    
    echo "${indent}[*regionalisation] "
    print_array REGIONALISATION
else
    echo "${indent}[regionalisation] "
    print_array REGIONALISATION
fi
...
function run_each_validation() {
    ...
    local EFFECT_SIZE_VALUES=
    
    eval "EFFECT_SIZE_VALUES=(\"\${$8[@]}\")"
    
    local REGIONALISATION_MATRICES=
    
    eval "REGIONALISATION_MATRICES=(\"\${$9[@]}\")"
    
    local WAIT_PERIOD=${10}
    local WAIT_INTERVAL="${11}"
    local SKIP_INVOCATIONS="${12}"
    local STOP_INVOCATIONS="${13}"
                ...
                for i in "${!GENERATORS[@]}"; do
                    ...
                    for t in "${!EXPERIMENT_TYPES[@]}"; do
                        ...
                        invocation="$run_replications \
                            ...
                            --effect-size \"{""${EFFECT_SIZE_VALUES[@]}""}\" \
                            --regionalisation \"{""${REGIONALISATION_MATRICES[@]}""}\" \
                        ...
                        if [[ $interval -eq $interval_limit ]]; then
                            ...
                            echo "Interval Limit: $interval_limit"
